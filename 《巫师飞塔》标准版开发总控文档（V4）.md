...
9. resolve newly covered visible wizards at destination（Model B 封印归属）
   9.1 check destination ground visible wizards covered by arriving bottom of moving slice
       -> 封进**覆盖塔**（movedSlice[0]），sealedAs = GROUND
   9.2 check destination top visible wizard(s) covered by arriving slice
       -> 封进**被覆盖塔**（dest 原顶塔 originalDestTopTower），sealedAs = COVERED_TOWER
   9.3 for all newly covered visible wizards:
       - set state = IMPRISONED（含 sealedAs）
       - attach them to the corresponding tower runtime (above)
...
Model B 解封（两种封印归属解封时机不同）：

1. inspect the source space after a tower slice has been removed;
   compute newTopTower = remaining stack top (after slice detached)
2. Part 1（被覆盖塔重新成顶）：
   2.1 for each wizard imprisoned inside newTopTower with sealedAs = COVERED_TOWER:
       - remove wizard from newTopTower.imprisonedWizards
       - new visible position = ON_TOWER_TOP on newTopTower (or ON_GROUND if no tower remains)
       - emit WIZARD_RELEASED
   2.2 非顶塔（仍被埋）的 COVERED_TOWER 封印不解封；newTopTower 的 GROUND 封印不解封
3. Part 2（覆盖塔移开其封印位置）：
   3.1 for each wizard imprisoned inside a moved-slice tower with sealedAs = GROUND
       and wizard.spaceIndex === sourceSpaceIndex:
       - remove wizard from that tower.imprisonedWizards
       - new visible position = ON_TOWER_TOP on newTopTower (or ON_GROUND)
       - emit WIZARD_RELEASED
   3.2 切片内的 COVERED_TOWER 封印不解封（随切片走、保持封印）

实现建议：
这里不要写成“猜测谁在最上面”。
应在运行时维护清晰的封印归属关系：某个巫师被封印在哪一座塔里、属于 COVERED_TOWER 还是 GROUND。
当被覆盖塔重新成为顶层（Part 1）、或覆盖塔移开其封印位置（Part 2）时，再把相应巫师恢复为可见状态。
...
1. collect all visible wizards at destination that become covered by movedSlice
   - ground wizards -> 将封进 covering tower (movedSlice[0])，sealedAs = GROUND
   - tower-top wizards -> 将封进 covered tower (dest 原顶塔)，sealedAs = COVERED_TOWER
2. if none -> return
3. for each covered wizard:
   3.1 remove wizard from visible position
   3.2 set wizard.state = IMPRISONED（含 sealedAs）
   3.3 attach wizard to the corresponding tower runtime (covered tower / covering tower)
   3.4 emit WIZARD_IMPRISONED（payload 含 sealedAs）
...
3. find the next position where:
   - 该位置是乌鸦纹章标记位（有塔时**顶塔**须带纹章；无塔时地面带纹章）
   - no visible wizard stands on the ground
   - no visible wizard stands on the tower top
4. if found:
   4.1 move raven castle there（有塔 -> 落到顶塔上 ON_TOWER；无塔 -> 落到地面 ON_SPACE）
