...
- 日期：2026-07-06
- 对应任务：T3.3
- 目标：创建一个包含棋盘、玩家信息和控制面板的完整游戏主界面。
- 产出物：
  - `packages/ui/src/screens/GameScreen.tsx` (新)
  - `packages/ui/src/components/PlayerInfo.tsx` (新)
  - `packages/ui/src/components/ControlPanel.tsx` (新)
  - `packages/ui/src/screens/GameScreen.css` (新)
- 执行步骤：
  1. ✅ **创建 `GameScreen.tsx`**: 作为顶层容器，使用 CSS Grid 布局，划分出玩家信息、棋盘和控制面板三个区域。
  2. ✅ **创建 `PlayerInfo.tsx`**: 用于展示单个玩家的状态，包括ID、是否为当前玩家、城堡内巫师数和药水状态。
  3. ✅ **创建 `ControlPanel.tsx`**: 用于展示全局控制选项，如当前回合信息、可用的法术列表和“结束回合”按钮。
  4. ✅ **集成组件**: 在 `GameScreen.tsx` 中组合 `Board`, `PlayerInfo`, 和 `ControlPanel` 组件，并通过 props 传递 `GameState` 和交互回调。
- 自检 / 不变量：
  - [x] `GameScreen` 组件能够渲染出一个完整的游戏界面。
  - [x] 玩家信息和控制面板能正确反映 `GameState` 的数据。
  - [x] 点击控制面板上的按钮（如法术、结束回合）会触发相应的回调函数。